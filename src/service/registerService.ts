...
import {IUser, NewRegister } from "../types/user";
...
export const createUser = async (appData: NewRegister) : Promise <IUser> => {
    ...
    const {rows} = await query("INSERT INTO users (name, email, password_hash,role, display_picture ) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
...
//Update role
export const updateUserRole = async(id: number, appData: IUser): Promise<IUser | null> => {
    const {role} = appData;
